...
class BeatScopeService:
    ...
    def get_project(self, request: GetProjectInput) -> dict[str, Any]:
        """One project's measured facts. No visual layer rides along.

        The handoff package ships timing facts and states no visual language,
        so this view reports the same scope: identity, counts, structure, and
        (on request) timing. Nothing here counts, names, or fingerprints a
        compiled visual artifact.
        """
    ...
    async def get_visual_state(self, request) -> dict[str, Any]:
        """The measured facts at one instant, via the shared runtime.

        This is ``track.at(time)`` and nothing more: the same call the web
        player makes, exposed by the exported package as ``getVisualState``.
        The name follows that package function - it is the state a visualizer
        samples, not a visual language. Scene, transition, and composition
        data are absent here exactly as they are absent from the package.
        """
