...
- When `response-relevance.json` is present and the manifest enables `response_relevance`, use `getResponseEvents(start, end, budget)` for effects or edits that cannot afford every onset. Choose the budget per shot or section; do not invent a global threshold. The selected objects are existing onsets and must keep their stored times.
...
`response_relevance` is an ordering value learned from human chart consensus. It is not probability, confidence, an instrument label, or a command to animate. If ranking is unavailable, `getResponseEvents` reports `chronological-fallback`; preserve that fact in diagnostics instead of presenting the fallback as ranked output.
