"""One-shot sealed evaluation for the strength-anchored v4 response ranker."""
...
EVALUATION_CONTRACT = "beatscope-sealed-ranking-evaluation-v4"
BOOTSTRAP_SEED_V4 = 20260907
LOCK_SCHEMA = "beatscope-response-ranking-candidate-lock-1"
REPORT_SCHEMA = "beatscope-response-ranking-evaluation-2"
...
def validate_lock(lock: object) -> list[str]:
    ...
    required = {
        "schema", "method", "candidate_sha256", "non_metric_candidate_sha256",
    ...
    digest_fields = required - {"schema", "method", "development_source_ids", "holdout"}
...
def evaluate_gates(report: dict) -> list[dict]:
    ...
    gate("pairwise_improvement", deltas["pairwise_accuracy"] is not None
         and deltas["pairwise_accuracy"] >= 0.02, f"delta={deltas['pairwise_accuracy']}")
    ...
    gate("ndcg_not_worse", deltas["ndcg_at_10"] is not None and deltas["ndcg_at_10"] >= -0.01,
         f"delta={deltas['ndcg_at_10']}")
    gate("sparse_recall_not_worse", deltas["recall_at_k"] is not None
         and deltas["recall_at_k"] >= -0.015, f"delta={deltas['recall_at_k']}")
    ...
    gate("non_metric_improvement", ranking["non_metric_delta_vs_strength"] >= 0.01,
    ...
    gate("source_slices_not_lost", bool(source_values)
         and all(value is not None and value >= -0.01 for value in source_values.values()),
    ...
    gate("format_slices_not_lost", bool(format_values)
         and all(value is not None and value >= -0.01 for value in format_values.values()),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (fresh_sources != {holdout["source_id"]}
        ...
        return 2
    visible_leakage = leakage_errors(manifest)
    if visible_leakage:
    ...
    bootstrap_result = er.paired_bootstrap(
        ...
        replicates=10000, seed=BOOTSTRAP_SEED_V4)
