...
def test_lock_contract_rejects_unknown_fields():
    lock = {
        ...
        "timeline_audit": {
            "development_count": 1, "holdout_count": 1, "overlap_count": 0,
            "development_set_sha256": "7" * 64, "holdout_set_sha256": "8" * 64,
        },
        "holdout": {
            "source_id": "holdout", "source_revision": "sha256:fixture",
            "license": "synthetic-original", "retained_song_count": 12,
            "retained_chart_count": 36,
            "excluded_development_overlap_count": 0,
            "excluded_audio_sha256s": [],
        },
        "surprise": True,
    }
...
def _sealed_fixture(tmp_path: Path, *, corrupt_lock: bool = False) -> dict[str, Path]:
    ...
    development_manifest = {
        "schema": er.MANIFEST_SCHEMA,
        "dataset_schema": er.DATASET_SCHEMA,
        "songs": [{"audio_sha256": "f" * 64, "fingerprint": "e" * 64,
                   "chart_sha256s": ["d" * 64], "split": "train"}],
        "charts": [],
    }
    development_path = tmp_path / "development-manifest.json"
    development_path.write_bytes(_bytes(development_manifest))
    development_sha = hashlib.sha256(development_path.read_bytes()).hexdigest()
    candidate["training"]["manifest_sha256"] = development_sha
    non_metric["training"]["manifest_sha256"] = development_sha
    ...
    return {"candidate": candidate_path, "non_metric": non_metric_path,
            "selection": selection_path, "development": development_path,
...
def _run(paths: dict[str, Path]) -> subprocess.CompletedProcess[str]:
    return subprocess.run([
        sys.executable, str(ROOT / "scripts" / "evaluate_event_ranker_v5.py"),
        ...
        "--development-manifest", str(paths["development"]),
...
def test_b2_gates_include_safety_and_quality_requirements():
    report = {
        ...
        "ranking": {
            "deltas_vs_strength": {"pairwise_accuracy": 0.005, "ndcg_at_10": 0.001,
                                    "recall_at_k": 0.001,
                                    "top_support_precision_at_k": 0.001},
            "bootstrap_pairwise": {"lower_95": 0.000001},
            "non_metric_delta_vs_strength": 0.005,
            "all_required_metrics_finite": True,
            "slices": {"highest_density_tertile": {"pairwise_delta_vs_strength": 0.001},
                       "sources": {"fresh": {"pairwise_delta_vs_strength": 0.0}},
                       "formats": {"stepmania": {"pairwise_delta_vs_strength": 0.0}}},
        },
...
def test_cross_corpus_leakage_checks_all_four_identity_classes():
    development = {
        "songs": [{"audio_sha256": "a", "fingerprint": "b", "chart_sha256s": ["c"]}],
        "charts": [{"timeline_sha256": "d"}],
    }
    holdout = json.loads(json.dumps(development))
    assert evaluation.cross_corpus_leakage_errors(development, holdout) == [
        "cross_corpus_audio_sha256", "cross_corpus_fingerprint",
        "cross_corpus_chart_sha256", "cross_corpus_timeline_sha256",
    ]


def test_frozen_r2d_lock_matches_the_approved_r2c_candidate():
    lock = json.loads((FIXTURES / "event_ranking_v6" / "candidate-lock.json").read_text())
    r2c = json.loads((FIXTURES / "event_ranking_v5" / "candidate-lock.json").read_text())
    assert evaluation.validate_lock(lock) == []
    assert lock["candidate_sha256"] == r2c["candidate_sha256"]
    assert lock["non_metric_candidate_sha256"] == r2c["non_metric_candidate_sha256"]
    assert lock["selection_report_sha256"] == r2c["selection_report_sha256"]
    assert lock["development_dataset_sha256"] == r2c["development_dataset_sha256"]
    assert lock["development_manifest_sha256"] == r2c["development_manifest_sha256"]
    assert lock["development_source_ids"] == r2c["source_ids"]
    assert lock["holdout"]["retained_song_count"] >= 12
    assert lock["holdout"]["retained_chart_count"] >= 36
